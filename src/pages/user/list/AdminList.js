...
const AdminList = () => {
  ...
  const getTitle = () => {
    if (isActiveUser) {
      return "تعداد مدیران فعال شما: "+activeUsers.length;
    } else if (isDeleteUser) {
      return "تعداد مدیران حذف شده: "+deletedUsers.length;
    }
  };
  ...
  return (
    <div>
      <BreadCrumbs
        title={"لیست مدیران هگزا اسکواد"}
        data={[{ title: "لیست مدیران", link: "/user/adminList" }]}
      />
      <Row>
        {/* <Col lg="3" sm="6">
          <StatsHorizontal
      ...
      <TableServerSide
        ...
        title={getTitle()}
        // BtnTitle={"اضافه کردن مدیران"}
        // BtnIcon={<User />}
        // btn={<AddUSer BtnTitle={"اضافه کردن مدیران"}  modalHeader={"افزودن مدیر جدید"}/>}
      />
    </div>
  );
};
